...
class AdvancedMetrics:
    """OPTIMIZED: P50/P95/P99 latencies, moving averages."""
...
class SegmentDeduplicator:
    ...
    @staticmethod
    def deduplicate(segments: List[TextSegment]) -> tuple:
        seen: Dict[str, int] = {}
...
class EnhancedTranslationPipeline(ITranslationPipeline):
    ...
    def __init__(
        ...
        if not engine or not parser_factory or not formatter_factory:
            raise ValidationError("Engine, parser_factory, formatter_factory required")
    ...
    def _translate_segments_optimized(
        ...
        if not should_batch:
            logger.warning("Circuit breaker open, using individual")
            if self._metrics:
    ...
    def _make_cache_key(self, segment: TextSegment, source_lang: str, target_lang: str, domain: str) -> str:
        s = f"{source_lang}:{target_lang}:{domain}:{segment.text}"
        return hashlib.sha256(s.encode('utf-8')).hexdigest()
    
    def _cache_result(self, segment: TextSegment, translated: str, source_lang: str, target_lang: str, domain: str):
        if not self._has_cache:
            return
        try:
    ...
    def _create_smart_batches(self, segments: List[TextSegment], target_size: int) -> List[List[TextSegment]]:
        batches = []
        current = []
        current_len = 0
        max_len = 4000
    ...
    def _validate_inputs(self, input_path: Path, output_path: Path, source_lang: str, target_lang: str, domain: str):
    ...
    def _format_document_safe(self, document: Document, output_path: Path, file_type: FileType):
        try:
