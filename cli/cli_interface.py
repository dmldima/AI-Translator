    try:
        ...
        if engine == 'openai':
            translation_engine = EngineRegistry.get_engine('openai', api_key=api_key, model=model)
        elif engine == 'deepl':
            translation_engine = EngineRegistry.get_engine('deepl', api_key=api_key, pro=deepl_pro)
        #!/usr/bin/env python3
"""
...
from src.core.models import TranslationJob, TextSegment, TranslationResult
...
def translate(input_file, output_file, source, target, engine, model, domain, api_key, deepl_pro):
    """
    ...
    if not api_key:
        console.print("[red]Error: API key required. Set OPENAI_API_KEY or use --api-key[/red]")
        sys.exit(1)
    ...
    console.print(f"[dim]Engine: {engine} ({model})[/dim]\n")
    ...
    try:
        ...
        if not translation_engine.validate_config():
            console.print("[red]Error: Invalid API configuration[/red]")
            sys.exit(1)
        ...
        pipeline = TranslationSystemFactory.create_pipeline(
            ...
            **({"model": model} if engine == "openai" else {"pro": deepl_pro})
        )
        ...
        if engine == 'openai':
            table.add_row("Tokens used", str(stats['total_tokens']))
        elif engine == 'deepl':
            table.add_row("Characters used", str(stats['total_chars']))
...
def test(engine, api_key):
    ...
    try:
        ...
        translation_engine = EngineRegistry.get_engine(engine, api_key=api_key)
...
def languages():
    ...
    """
    from src.core.models import SUPPORTED_LANGUAGES
...
def formats():
    ...
    parsers = ParserFactory.get_supported_types()
    formatters = FormatterFactory.get_supported_types()
...
@click.option('--api-key', help='API key')
@click.option('--deepl-pro', is_flag=True, help='Use DeepL Pro')
def batch(input_dir, output_dir, source, target, engine, pattern, api_key, deepl_pro):
    """
    ...
    try:
        ...
        pipeline = TranslationSystemFactory.create_pipeline(
            ...
            glossary_enabled=False,
            **({"pro": deepl_pro} if engine == "deepl" else {})
        )
...
if __name__ == '__main__':
    cli()
        ...
        # Create pipeline
        pipeline = TranslationPipeline(engine=translation_engine)
...
def text(text, source, target, engine, model, api_key):
    ...
    try:
        # Initialize
        if engine == 'openai':
            translation_engine = OpenAIEngine(api_key=api_key, model=model)
        else:
...
def test(engine, api_key):
    ...
    try:
        if engine == 'openai':
            translation_engine = OpenAIEngine(api_key=api_key)
        else:
            console.print(f"[red]Unsupported engine: {engine}[/red]")
            sys.exit(1)
...
def formats():
    ...
    formats_info = {
        FileType.DOCX: ("Microsoft Word", "Full formatting support"),
        FileType.DOC: ("Microsoft Word (Legacy)", "Limited support"),
        FileType.XLSX: ("Microsoft Excel", "Coming soon"),
        FileType.XLS: ("Microsoft Excel (Legacy)", "Coming soon"),
        FileType.PDF: ("PDF Document", "Coming soon"),
        FileType.TXT: ("Plain Text", "Coming soon")
    }
    ...
    for file_type, (format_name, status) in formats_info.items():
        status_style = "green" if "support" in status else "yellow"
        table.add_row(
            f".{file_type.value}",
            format_name,
...
@click.option('--target', '-t', required=True, help='Target language code')
@click.option('--engine', '-e', default='openai', help='Translation engine')
@click.option('--pattern', '-p', default='*.docx', help='File pattern to match')
@click.option('--api-key', envvar='OPENAI_API_KEY', help='API key')
def batch(input_dir, output_dir, source, target, engine, pattern, api_key):
    """
    Translate multiple documents in a directory.
